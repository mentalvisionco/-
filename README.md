...
graph TD
    ...
    Routes -->|Winston Logger & Rotate| Logs[(ملفات السجلات الدورية)]
```
...
│   └── package.json
├── logs/                  # سجلات Winston الدورية (combined.log, error.log)
├── services/
│   ├── logger.js          # إعداد وإدارة Winston للتدقيق والتدوير اليومي
...
تم دمج نظام التسجيل `Winston` لتخزين ومراقبة سلوك التطبيق:
- **سجل الأخطاء (`logs/error-[DATE].log`):** يسجل الأخطاء وتتبع مكدس الأخطاء (Stack Traces) فقط.
- **السجل العام (`logs/combined-[DATE].log`):** يسجل كافة الأنشطة بصيغة JSON منظمة.
- **تدوير الملفات:** تدوير يومي تلقائي يحتفظ بالملفات لمدة **14 يومًا** وبحد أقصى **20 ميجابايت** لكل ملف.
- **سجل التدقيق (Audit Trail):** يسجل العمليات الحساسة مثل تغيير كلمات المرور، وحذف الطلاب، وتعديل جلسات الحضور في جدول `audit_logs` بقاعدة البيانات مباشرة لتوثيق إجراءات الإدارة.
